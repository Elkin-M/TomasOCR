# Single File Application: FichaProcessor_V6_1_Credits_And_GUI_API_Key
...
DEFAULT_PROCESSED_IMAGES_BASE_FOLDER = "."          # Base for output folders (reports, processed_api images)
DEFAULT_INPUT_FILES_FOLDER = "fichasatrascribir"    # Default location for user's PDF/Excel
# Load .env for development convenience or if user prefers it.
# The GUI field for API key will take precedence if filled.
load_dotenv()
...
def convertir_pdf_a_imagenes_consecutivas_jpg(archivo_pdf, ruta_salida, dpi=300, logger_func=None):
    ...
    try:
        ...
        for num_pagina in range(len(pdf_documento)):
            ...
            nombre_archivo_salida = os.path.join(ruta_salida, f"imagen{num_pagina + 1}.jpg")
...
def parse_expiry_date(date_str, logger_func=None):
    ...
    if not date_str or not isinstance(date_str, str):
        return None
    formats_to_try = ["%d/%m/%Y", "%d-%m-%Y", "%Y/%m/%d", "%Y-%m-%d", "%d %b %Y", "%d %B %Y", "%Y%m%d"]
...
def process_images_with_mistral(image_folder_path, ficha_id, api_key_to_use, logger_func=None): # Added api_key_to_use
    ...
    destination_folder_name = f"{ficha_id}_{timestamp_str}_procesados_api"
    ...
    # API key is now passed as an argument: api_key_to_use
    if not api_key_to_use: # Should be checked before calling this function, but a safeguard
        msg = "Error Interno: API Key no fue pasada a process_images_with_mistral."
        log_message_to_gui(msg, logger_func)
        return [], [], [], None

    log_message_to_gui("\n--- Iniciando Procesamiento de Imágenes con Mistral ---", logger_func)
    for local_image_path in files_to_process:
        ...
        messages = [{"role": "user", "content": [{"type": "text", "text": text_prompt}, {"type": "image_url", "image_url": image_input_for_api}]}]
        
        max_attempts, attempt, interpretation_result, api_call_successful, last_api_error = 2, 0, None, False, None
        while attempt < max_attempts:
            attempt += 1
            try:
                client = Mistral(api_key=api_key_to_use) # Use passed API key
                log_message_to_gui(f"Intento {attempt}/{max_attempts} API para '{os.path.basename(local_image_path)}'", logger_func)
                chat_response = client.chat.complete(model=model, messages=messages)
                if not chat_response.choices: raise ValueError("Respuesta API inválida")
        ...
        try:
            ...
            datos['fuente_imagen_original'] = os.path.basename(local_image_path)
            
            doc_type = datos.get('tipo_documento', '').strip().lower()
            expiry_date_str = datos.get('fecha_vencimiento', None)
            datos['estado_vencimiento'] = "No Aplica"
...
def process_data_and_compare_excel(lista_de_datos_api, ficha_id, excel_file_path, logger_func=None):
    ...
    if not lista_de_datos_api:
        ...
        try:
            ...
            for col in api_cols_expected_internal:
    ...
    try:
        if not df_api.empty:
            ...
            if existing_cols_for_save:
                 df_api[existing_cols_for_save].to_excel(intermediate_filepath, sheet_name="Datos_API_Extraidos_Detalle", index=False)
    ...
    try:
        with pd.ExcelWriter(final_report_path, engine='openpyxl') as writer:
            ...
            if sheet_written_count == 0 and df_api.empty:
                 pd.DataFrame([{"info":"No se extrajeron datos de API válidos."}]).to_excel(writer, sheet_name="API_Datos_Detallados", index=False); sheet_written_count +=1
            ...
            if not df_api.empty and not df_excel.empty and \
               ...
               all(col in df_excel.columns for col in ['identificacion_excel', 'nombre_norm_excel']):
                ...
                df_merged = pd.merge(df_api_simple, df_excel_simple, on='identificacion', how='outer', suffixes=('_lista', '_excel'), indicator=True )
                ...
                if name_mismatch_cond.any():
                    ...
                    df_nm.drop_duplicates(subset=['identificacion'], keep='first', inplace=True); df_discrepancies_list_for_concat.append(df_nm)
                ...
                if only_list_cond.any():
                    ...
                    df_ol.rename(columns={'nombre_lista': 'nombre_en_api'}, inplace=True); df_ol['nombre_en_plataforma'] = pd.NA
                    df_ol.drop_duplicates(subset=['identificacion'], keep='first', inplace=True); df_discrepancies_list_for_concat.append(df_ol)
                ...
                if only_excel_cond.any():
                    ...
                    df_oe['nombre_en_api'] = pd.NA; df_oe.rename(columns={'nombre_excel': 'nombre_en_plataforma'}, inplace=True)
                    df_oe.drop_duplicates(subset=['identificacion'], keep='first', inplace=True); df_discrepancies_list_for_concat.append(df_oe)
            ...
            if sheet_written_count == 0:
                pd.DataFrame([{"info":"No hay datos suficientes para informe."}]).to_excel(writer, sheet_name='Resumen_General', index=False)
...
class FichaProcessorApp:
    def __init__(self, root_window):
        ...
        self.root.title("Procesador de Fichas PDF v6.1 (API Key GUI & Créditos)")
        self.root.geometry("950x890")
        ...
        self.api_key_var = tk.StringVar(value=os.getenv("MISTRAL_API_KEY", ""))
        ...
        help_menu.add_separator(); help_menu.add_command(label="Salir", command=self.root.quit)

        api_key_frame = ttk.LabelFrame(self.root, text="Configuración API Key Mistral", padding=(10,5)); api_key_frame.pack(padx=10, pady=(10,0), fill="x")
        ttk.Label(api_key_frame, text="Mistral API Key:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        self.api_key_entry = ttk.Entry(api_key_frame, textvariable=self.api_key_var, width=60, show="*"); self.api_key_entry.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        self.test_api_button_top = ttk.Button(api_key_frame, text="Probar API Key", command=self.test_api_connection_thread); self.test_api_button_top.grid(row=0, column=2, padx=5, pady=5)
        ...
        credits_label_text = "Desarrollado por Gerardo Esquivia y Tomas Venegas - SENA CBPO - CTI - 2025"
        self.credits_label = ttk.Label(self.root, text=credits_label_text, anchor="center", font=("Arial", 8))
        self.credits_label.pack(side=tk.BOTTOM, fill="x", pady=(5,5), padx=10)

        self.log_queue = queue.Queue(); self.root.after(100, self.process_log_queue)
        if not self.api_key_var.get(): self.gui_log_direct("ADVERTENCIA: MISTRAL_API_KEY no configurada.")
        else: self.gui_log_direct("MISTRAL_API_KEY encontrada/ingresada.")
        self.gui_log_direct("Bienvenido. Ingrese API Key si es necesario y seleccione PDF para Paso 1.")
        self._update_button_states()

    def show_about_dialog(self):
        about_title = "Acerca del Procesador de Fichas PDF"
        about_message = ("Procesador de Fichas PDF v6.1\n\nDesarrollado por:\nGerardo Esquivia\nTomas Venegas\n\nInstitución:\nCentro de Comercio y Servicios\nRegional Bolívar - SENA Colombia\n\nAño: 2025\n\nEste software utiliza la API de Mistral.")
    ...
    def _update_button_states(self, is_processing_any_step=False):
        ...
        self.test_api_button_top.config(state=tk.NORMAL if not is_processing_any_step else tk.DISABLED)
        ...
        if is_processing_any_step:
            ...
            self.step2_mistral_and_excel_button.config(state=tk.NORMAL if self.pdf_to_images_step_done and api_key_present else tk.DISABLED)
    ...
    def start_mistral_and_comparison_thread(self):
        gui_api_key = self.api_key_var.get().strip()
        env_api_key = os.getenv("MISTRAL_API_KEY", "").strip()
        effective_api_key = gui_api_key if gui_api_key else env_api_key
        if not effective_api_key: messagebox.showerror("Error API Key", "Ingrese API Key o configúrela en .env."); self.gui_log_direct("API Key no proporcionada para Paso 2."); return
        if not self.pdf_to_images_step_done or not self.current_temp_images_path: messagebox.showerror("Error de Flujo", "Complete 'Paso 1' primero."); return
        if not self.ficha_id: messagebox.showerror("Error Interno", "Ficha ID no definida."); return
        ...
        threading.Thread(target=self._run_resequence_mistral_and_comparison, args=(effective_api_key,), daemon=True).start()

    def _run_resequence_mistral_and_comparison(self, api_key_to_use):
        try:
            ...
            if not resequence_success: messagebox.showerror("Error Resecuenciación", "No se pudieron resecuenciar. Revise log."); self._update_progress(0, "Error resecuenciando."); return # Added return
            if num_resequenced == 0: messagebox.showinfo("Sin Imágenes", "No quedaron imágenes tras limpieza/resecuenciación."); self._update_progress(0, "No hay imágenes."); return # Added return
            ...
            api_data, err_p, err_r, processed_api_img_dir = process_images_with_mistral(self.current_temp_images_path, self.ficha_id, api_key_to_use, logger_func=self.gui_log_from_thread)
    ...
    def test_api_connection_thread(self):
        ...
        env_api_key = os.getenv("MISTRAL_API_KEY", "").strip()
        effective_api_key = gui_api_key if gui_api_key else env_api_key
        if not effective_api_key: messagebox.showerror("Error API Key", "Ingrese API Key o configúrela en .env."); self.gui_log_direct("Prueba API Fallida: API Key no proporcionada."); return
        self.gui_log_direct("Iniciando prueba API..."); self._update_button_states(True); self._update_progress(0, "Probando API...")
        threading.Thread(target=self._run_api_test, args=(effective_api_key,), daemon=True).start()
    def _run_api_test(self, api_key_to_use):
        self.gui_log_from_thread(f"Intentando conectar con Mistral API...");
        try:
            client = Mistral(api_key=api_key_to_use); models = client.models.list()
